...
class autoShorts:
    ...
    def downloadBackgroudVidoes(videoBackgroundLinksFileName: str, videoBackgroundFolder: str) -> None:
        ...
        if len(videoBackgroundLinkList) <= 0:
            autoShorts.exitHandler(f"No videos in \"{videoBackgroundLinkList}\" file")
        for videoLink in videoBackgroundLinkList:
            try:
                ...
                try:
                    ...
                    autoShorts.exitHandler("ERROR: Failed to save video")
        autoShorts.exitHandler("Download Complete")
        ...
        for model in modelList:
            if model.find(language) == 11:
                try:
                    ...
                    autoShorts.createAudioFile(model, "My name is, " + model.replace("/", " ").replace("_", " ").replace("-", " "), settings._language, settings._speed, f"{programDirectory}/sample.wav")
    ...
    def makeBackgroundVideoSegment(videoBackgroundFolder: str, outputAudioName: str) -> None:
        ...
        if len(backgroundVideoList) <= 0:
            autoShorts.exitHandler(f"No videos in \"{videoBackgroundFolder}\" folder")
...
if __name__ == "__main__":
    autoShorts.startUpScreen()
    
    atexit.register(autoShorts.crashHandler)
    ...
    if settings["downloadVideosInLinksFile"]:
        autoShorts.downloadBackgroudVidoes(settings["videoBackgroundLinksFileName"], settings["videoBackgroundFolderName"])
    if settings["loopAllModels"]:
        autoShorts.loopAllModels(settings["language"], settings["outputAudioName"])
        loadingClass.endLoadingAnimation()
        autoShorts.exitHandler()
    else:
        for i in range(0, len(scriptFileList)):
            script = autoShorts.getScripFile(programDirectory, settings["scriptsFolderName"], scriptFileList[i])
            deleteGeneratedFilesClass.deleteGeneratedFiles()
            autoShorts.createAudioFile(settings["ttsModel"], script, settings["language"], settings["speed"], settings["outputAudioName"])
            ...
            autoShorts.makeBackgroundVideoSegment(settings["videoBackgroundFolderName"], settings["outputAudioName"])
        ...
        deleteGeneratedFilesClass.deleteGeneratedFiles()
        loadingClass.endLoadingAnimation()
        autoShorts.exitHandler("All Tasks Completed")
else:
    pass